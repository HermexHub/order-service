...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
	...
	private async setupTopology(): Promise<void> {
		...
		// Assert Order Service Consumer Queue for Saga events
		await this.channel.assertQueue(RabbitQueues.ORDER_PAYMENT_EVENTS, {
			durable: true,
		...
		await this.channel.bindQueue(
			RabbitQueues.ORDER_PAYMENT_EVENTS,
		...
		this.logger.log('RabbitMQ exchanges and queues topology verified')
	}
	...
	async consumeEvents<T>(
		...
		await this.channel.consume(queueName, async (msg) => {
			...
			try {
				...
				this.logger.log(
					`[AMQP Sub] Processing message from ${queueName} (Correlation: ${correlationId})`
				)
				...
				// Negative acknowledgement without requeue after max retry / DLQ
